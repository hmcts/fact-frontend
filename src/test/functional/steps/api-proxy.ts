...
When(
  'I make an API call to {string} and expect some JSON to be returned',
...
When(
  'I make an API call to {string} and expect some header and slug {string} to be returned',
  async function (path: string, slug: string) {
    const response = await Axios.get(testConfig.TEST_URL + path);
    expect(response.headers['content-type']).equal('application/json');
