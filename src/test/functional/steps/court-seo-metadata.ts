...
Given(
  'a {string} page with the court slug {string} loads',
  async function (courtType: string, courtSlug: string) {
...
Then(
  'the page should contain a script element containing structured data',
  async () => {
...
Then('it should be parseable into a JSON object', async () => {
  const structuredData = await I.grabTextFrom(
