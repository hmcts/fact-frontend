...
When('I select It is not listed here', async () => {
  ...
  I.click(element);
  I.click(
    "button[class='govuk-button govuk-!-margin-top-2 govuk-!-margin-bottom-8 continue']",
  );
});
...
When(
  'I click on the link Search for a court by prefix \\(A - Z)',
  async function () {
...
When('I click on the letter {string}', async (character: string) => {
  ...
  I.click(`#alphabet-buttons > a:nth-child(${charIndex})`);
});

Then(
  'I can see courts list all start with {string}',
  async (alphabet: string) => {
    const courtHtmlElement = await I.grabTextFromAll('#results-list > h2 > a');
    expect(courtHtmlElement.length > 0).equal(true);
    courtHtmlElement.forEach((courtName) =>
      expect(courtName.startsWith(alphabet)).equal(true),
    );
    const sortedCourtNames = courtHtmlElement.sort();
    let isEqual = true;
    for (let i = 0; i < courtHtmlElement.length; ++i) {
      if (courtHtmlElement[i] === sortedCourtNames[i]) isEqual = true;
      else {
        isEqual = false;
        break;
      }
