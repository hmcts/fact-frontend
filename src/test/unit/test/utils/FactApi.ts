...
describe('FactApi', () => {
  ...
  test('Should return results from get request for old court name', async () => {
    ...
    await expect(
      api.searchCourtNameHistory('SOME_OLD_COURT_NAME', 'en'),
  ...
  test('Should return no result and log error from get request for old court name', async () => {
    ...
    await expect(api.searchCourtNameHistory(null, 'en')).resolves.toEqual({
      null: [],
      error: true,
    });
    await expect(spy).toBeCalled();
  ...
  test('Should return list of court from postcode and aol search', async () => {
    ...
    await expect(
      api.postcodeServiceAreaSearch('AA9 9AA', 'aol', '', 'en'),
    ).resolves.toEqual(results.data);
  });
  ...
  test('Should return no courts and log error', async () => {
    ...
    await expect(
      api.postcodeServiceAreaSearch('AA9 9AA', 'aol', '', 'en'),
    ).resolves.toEqual({ courts: [], error: true });
    await expect(spy).toBeCalled();
  });
  ...
  test('Should return list of court from a postcode search', async () => {
    ...
    await expect(api.postcodeAreaSearch('AA9 9AA', 'en')).resolves.toEqual(
      results.data,
    );
  });
  ...
  test('Should return no courts from a court name prefix search and log error', async () => {
    ...
    await expect(api.courtPrefixSearch('A')).resolves.toEqual({
      results: [],
      error: true,
    });
    await expect(spy).toBeCalledTimes(1);
