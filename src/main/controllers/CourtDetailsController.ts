...
export class CourtDetailsController {
  ...
  public async get(req: FactRequest, res: Response, next: NextFunction) {
    ...
    if (!isEmpty(slug)) {
      const courtDetails: CourtDetailsResult = await this.api.court(
        slug,
        req.lng,
      );
      ...
      if (!isObjectEmpty(courtDetails)) {
        if (courtDetails.open) {
          const viewData: CourtDetailsData = cloneDeep(
            req.i18n.getDataByLanguage(req.lng)['court-details'],
          ) as CourtDetailsData;
          viewData.title = viewData.title.replace(
            '{courtName}',
            courtDetails.name,
          );
          ...
          const enquiries: Enquiries = {
            phone: filterByDescription(courtDetails.contacts, [
              'enquiries',
              'ymholidau',
            ]),
            welshPhone: filterByDescription(courtDetails.contacts, [
              'welsh',
              'cymraeg',
            ]),
            emails: filterByDescription(courtDetails.emails, [
              'enquiries',
              'ymholidau',
            ]),
            fax: filterByDescription(courtDetails.contacts, ['fax', 'ffacs']),
            sendDocumentsEmail: filterByDescription(courtDetails.emails, [
              'send documents',
              'anfon dogfennau',
            ]),
          };
          ...
          viewData.seoMetadataDescription = (
            viewData.seoMetadataDescription as string
          ).replace('{courtName}', courtDetails.name);
          ...
          const viewData: any = cloneDeep(
            req.i18n.getDataByLanguage(req.lng)['closed-court'],
          );
          viewData.title = viewData.title.replace(
            '{courtName}',
            courtDetails.name,
  ...
  private imageHandler(courtDetails: CourtDetailsResult): void {
    if (courtDetails['image_file']) {
      courtDetails['image_file'] =
        config.get('services.image-store.url') +
        '/' +
        courtDetails['image_file'];
    }
  }
   ...
   * @params courtDetails CourtDetailsResult,
   */
  private getIntroParagraph(
    req: FactRequest,
    courtDetails: CourtDetailsResult,
  ): string {
    return req.lng == 'en'
      ? courtDetails.service_centre.intro_paragraph
      : courtDetails.service_centre.intro_paragraph_cy;
  }
  /**
   ...
   */
  private replaceCatchmentAndServiceArea(
    viewData: CourtDetailsData,
    courtDetails: CourtDetailsResult,
  ): string {
    return viewData.notInPersonP1
      .replace(
        '{catchmentArea}',
        decideCatchmentArea(this.regionalCentre, viewData.catchmentArea),
      )
  ...
  public setNotInPersonP1(
    ...
    if (courtDetails['service_centre']) {
      if (courtDetails.service_centre.intro_paragraph.length > 0) {
        ...
        viewData.notInPersonP1 = this.replaceCatchmentAndServiceArea(
          viewData,
          courtDetails,
        );
      }
