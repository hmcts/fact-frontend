...
export class ProxyMiddleware {
  enableFor(app: Application): void {
    ...
    // V2 Endpoints Proxy
    app.use('/v2/proxy/search/slug/:slug', createProxyMiddleware({
      changeOrigin: true,
    ...
    app.use('/v2/proxy/search/court-types/:courtTypes', createProxyMiddleware({
      changeOrigin: true,
      target: config.get('services.api.url'),
      pathRewrite: (path, req) => {
    ...
    app.use('/v2/proxy/search/postcode/:postcode/serviceArea/:serviceArea', createProxyMiddleware({
      changeOrigin: true,
      target: config.get('services.api.url'),
      pathRewrite: (path, req) => {
