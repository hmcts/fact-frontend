...
import {
  SearchCourtHistoryResult,
  SearchResult,
} from '../interfaces/SearchResultsData';
import { CourtDetailsResult } from '../interfaces/CourtDetailsData';
import {
  CourtWithDistance,
  PostcodeSearchResultsData,
...
export class FactApi {
  ...
  public searchCourtNameHistory(
    query: string,
    lng: string,
  ): Promise<SearchCourtHistoryResult> {
    return this.axios
      .get(`/courts/court-history/search?q=${query}`, {
  ...
  public postcodeServiceAreaSearch(
    postcode: string,
    serviceAreaSlug: string,
    action: string,
    lng: string,
  ): Promise<PostcodeSearchResultsData> {
    return this.axios
      .get(
        `search/results?postcode=${postcode}&serviceArea=${serviceAreaSlug}&action=${action}`,
        { headers: { 'Accept-Language': lng } },
      )
      .then((results) => results.data)
      .catch((err) => {
  ...
  public postcodeAreaSearch(
    postcode: string,
    lng: string,
  ): Promise<Array<CourtWithDistance>> {
    return this.axios
      .get(`search/results/${postcode}`, {
        headers: { 'Accept-Language': lng },
      })
      .then((results) => results.data)
      .catch((err) => {
